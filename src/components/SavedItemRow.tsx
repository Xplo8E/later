...
export default function SavedItemRow({ link, selected, compact, onSelect, onPatch, onDelete, onEdit }: {
  link: SavedLink; selected: boolean; compact?: boolean;
  onSelect: (link: SavedLink) => void; onPatch: (id: string, patch: LinkPatch) => Promise<void>;
  onDelete: (link: SavedLink) => void; onEdit: (link: SavedLink, field: 'note' | 'tags') => void;
}) {
  return <article className={`saved-row ${selected ? 'selected' : ''} ${compact ? 'compact' : ''}`}>
    <button className="row-main" onClick={() => onSelect(link)} aria-label={`View ${link.title}`}>
      <Thumbnail link={link} />
      <span className="row-copy"><span className="row-title">{link.status === 'finished' && <span className="finished-check" aria-hidden="true"><Check /></span>}{link.title}</span>
        <span className="row-source">{link.domain}{link.readingMinutes ? ` · ${link.readingMinutes} min` : ''}</span>
        {!compact && (link.status === 'finished' ? <span className="row-note">Finished {dateLabel(link.finishedAt || link.savedAt)}</span> : link.note && <span className="row-note">{link.note}</span>)}
    ...
    <DropdownMenu.Root><DropdownMenu.Trigger asChild><button className="row-menu icon-button" aria-label={`Actions for ${link.title}`}><MoreHorizontal aria-hidden="true" /></button></DropdownMenu.Trigger>
      <DropdownMenu.Portal><DropdownMenu.Content className="dropdown" align="end" sideOffset={5}>
        ...
        <DropdownMenu.Separator /><DropdownMenu.Item className="danger-text" onSelect={() => onDelete(link)}>Delete</DropdownMenu.Item>
