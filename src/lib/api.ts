...
async function request<T>(path: string, method = 'GET', body?: unknown, signal?: AbortSignal): Promise<T> {
  ...
  // Access can return a login document instead of the expected API JSON.
  if (!response.headers.get('content-type')?.includes('application/json')) {
    throw new ApiError('Your session expired. Sign in again.', 401);
  }
  const result = await response.json();
  if (!response.ok) {
    throw new ApiError(result.error || 'Could not complete this request.', response.status, result);
  }
  return result as T;
}
...
export const api = {
  ...
  async exportLibrary(): Promise<Blob> {
    ...
    if (!response.ok || !response.headers.get('content-type')?.includes('application/json')) {
      throw new ApiError('Could not export your library. Try signing in again.', response.status);
    }
