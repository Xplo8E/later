...
function quote(value: string | null | number): string {
...
for (const link of fixtures) {
  // Keep values in the same order as the column names.
  const columns = [
    'id', 'url', 'normalized_url', 'title', 'domain', 'kind', 'note', 'status',
    'saved_at', 'updated_at', 'finished_at', 'reading_minutes', 'metadata_status',
  ];
  const values = [
    link.id, link.url, link.normalizedUrl, link.title, link.domain, link.kind, link.note,
    link.status, link.savedAt, link.updatedAt, link.finishedAt, link.readingMinutes, 'ready',
  ];
...
try {
  ...
  execFileSync(process.execPath, [
    'node_modules/wrangler/bin/wrangler.js', 'd1', 'execute', 'DB', '--local', '--file', filename,
  ], { stdio: 'inherit' });
} finally {
  // Remove the temporary SQL even if Wrangler fails.
