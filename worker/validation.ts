...
export function savedUrl(value: unknown): URL {
  ...
  let url: URL;
  try { url = new URL(input); } catch { throw new HttpError(400, 'Enter a complete http:// or https:// URL.'); }
...
export function cleanTags(value: unknown): string[] {
  ...
  return value.map(value => text(value, 'Tag', 40, false)).filter(tag => { const key = tag.toLocaleLowerCase('en-US'); if (seen.has(key)) return false; seen.add(key); return true; });
}
export function parsePatch(body: Record<string, unknown>): LinkPatch {
  ...
  if ('status' in body) { if (!statuses.includes(body.status as LinkStatus)) throw new HttpError(400, 'Invalid link status.'); patch.status = body.status as LinkStatus; }
  return patch;
}
export function parseSettings(body: Record<string, unknown>): Partial<Settings> {
  ...
  for (const key of ['fetchMetadata', 'suggestTags']) if (key in body && typeof body[key] !== 'boolean') throw new HttpError(400, `${key} must be a boolean.`);
